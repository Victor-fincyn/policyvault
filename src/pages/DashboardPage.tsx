...
export function DashboardPage() {
  ...
  return (
    <div className="min-h-full pb-24 bg-[#F5F4FB]">
      {/* Hero Header */}
      <div className="bg-gradient-to-br from-[#1e1c38] via-[#313057] to-[#4a4878] px-4 pt-10 pb-10 relative overflow-hidden">
        {/* Decorative blobs */}
          ...
          <Card className="shadow-md overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-3">
              <div>
                ...
                <p className="text-[12px] text-[#7878A8] mt-0.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => scrollTL(-1)}
                  className="w-7 h-7 rounded-full border border-[#C8C7E8] bg-white flex items-center justify-center text-[#7C79C8] hover:bg-[#F5F4FB] active:scale-95 transition-all shadow-sm"
                >
            ...
            >
              <div
                className="relative select-none bg-[#ECEAFB]"
                style={{ width: tlTotalWidth, height: TL_H }}
              >
                {/* Baseline */}
                <div
                  className="absolute"
                  style={{
                    ...
                    height: 2,
                    background:
                      "linear-gradient(to right, transparent, #C8C7E8 6%, #C8C7E8 94%, transparent)",
                  }}
                />
                {/* Today marker */}
                <div
                ...
                <div
                  className="absolute -translate-x-1/2"
                  style={{ left: TL_LEFT_PAD, top: TL_LINE_Y - 30 }}
                >
                  <span className="bg-[#7C79C8] text-white text-[9px] font-extrabold tracking-wider px-2 py-0.5 rounded-full shadow">
                    TODAY
                  </span>
                </div>
                {/* Gap labels on line */}
                {tlEvents.map((event, i) => {
                  const prevX =
                    i === 0 ? TL_LEFT_PAD : (tlPositions[i - 1] ?? TL_LEFT_PAD);
                  const x = tlPositions[i] ?? TL_LEFT_PAD;
                  const midX = (prevX + x) / 2;
                  const gap =
                    i === 0 ? event.days : event.days - tlEvents[i - 1].days;
                  const gapLabel =
                    gap <= 0
                      ? null
                      : gap > 90
                        ? Math.round(gap / 30) + " mo"
                        : gap + "d";
                  return gapLabel ? (
                    <div
                      key={"gap-" + event.id}
                      className="absolute -translate-x-1/2 -translate-y-1/2 bg-[#ECEAFB] px-1.5 text-[10.5px] font-bold text-[#7C79C8] whitespace-nowrap"
                      style={{ left: midX, top: TL_LINE_Y }}
                    >
                      {gapLabel}
                    </div>
                  ) : null;
                ...
                {tlEvents.map((event, i) => {
                  ...
                  const labelTop = isAbove
                    ? dotTop - STEM_LEN - LABEL_H
                    : TL_LINE_Y + TL_DOT_R + STEM_LEN;
                  const stemTop = isAbove
                    ? labelTop + LABEL_H
                    : TL_LINE_Y + TL_DOT_R;
                  const urgentColor =
                    event.days < 0
                  ...
                  const daysLabel =
                    event.days < 0
                      ? Math.abs(event.days) + "d ago"
                      : event.days === 0
                        ? "Today"
                        : event.days > 90
                          ? Math.round(event.days / 30) + " mo"
                          : event.days + " days";
                  return (
                    <React.Fragment key={event.id}>
                      <div
                        className="absolute bg-[#C8C7E8]"
                        style={{
                          left: x - 0.75,
                          top: stemTop,
                          width: 1.5,
                      ...
                      <div
                        className={
                          "absolute flex items-center justify-center rounded-full bg-white border-2 shadow-sm cursor-pointer hover:scale-110 active:scale-95 transition-transform z-10 " +
                          dotBorder
                        }
                        style={{
                          left: x - TL_DOT_R,
                        ...
                        onClick={() =>
                          navigate("/policies/" + event.policyId + "?from=/")
                        }
                      >
                      ...
                      >
                        <div className="text-[13px] font-bold text-[#313057] leading-tight">
                          {event.label}
                        </div>
                        <div className="text-[12px] font-semibold text-[#313057] leading-tight mt-0.5">
                          {event.sublabel}
                        </div>
                        <div className="text-[11px] text-[#7878A8] leading-tight mt-1">
          ...
          <Card className="p-4 shadow-md">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-[#7C79C8] text-sm font-semibold flex items-center gap-0.5"
              >
              ...
              {typeCounts.map(({ type, count }, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-[#313057]">
                      {getTypeLabel(type)}
                    </span>
                  </motion.button>
                );
